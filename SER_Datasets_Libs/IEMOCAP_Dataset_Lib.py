...
def create_IEMOCAP_file_objects(db_path):
    ...
    Emotions_format = {'N':'neu', 'H':'hap', 'S':'sad', 'A':'ang', 'F':'fea', 'D':'dis', 'U':'sur', 'R':'fru', 'E':'exc'} #'X':'xxx'
                    ...
                    if(len(splitted) > 5):
                        for emo in Emotions_format:
                            if(splitted[4]==Emotions_format[emo]):
                                ...
                                #print(wav_file)
                                this_clip_info = Clip_file_Class(wav_file, speaker_id=speaker_id, accent=1, sex=splitted[3][-4], emotion=emo, intensity=1, statement=int(splitted[3][-3:]), repetition=1, db_id=4)
